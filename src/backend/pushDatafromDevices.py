...
def getDataFromTestPost(rawData):
    ...
    try:
        ...
        try:
            ...
            subOtherInfo['openPoint1'] = round(float(str(struct.unpack('f', rawData[30:34]))[0:-2]), 3)
            print('openPoint1', subOtherInfo['openPoint1'])
            subOtherInfo['openPoint2'] = round(float(str(struct.unpack('f', rawData[34:38]))[0:-2]), 3)
