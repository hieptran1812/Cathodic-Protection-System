...
while True:
    try:
        ...
        while True:
            ...
            # startPacket, locationSystem, centralAddress, notUse, notUse, dienApPin, dienApNguon, temperature, dienAC3Pha, dienDCPoint1, dongDienDC, notUse, sdt, signal= struct.unpack('<HHHdH', data[:13])
            # print(startPacket, locationSystem, centralAddress, notUse, notUse)
            startPacket, locationSystem, centralAddress = struct.unpack('BBBBBB', data[:6])
            print(startPacket)
            print(locationSystem)

            # data = connection.recv(2) # number of bytes
            # # print(data)
            # # print(len(data))
            # # locationSystem, centralAddress = struct.unpack("<II", read_val[2:])
            # # print(locationSystem, centralAddress)
