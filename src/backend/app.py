...
def getDashboard():
  ...
  for doc in db.RectifierTransformersDetails.find({}, {'_id':1,'devSerial':1,'maChuoi':1, 'organization': 1, 
                                                       'dateUpdate':1, 'date':1,
  ...
  for doc in db.TestPostsDetails.find({}, {'_id':1,'devSerial':1,'maChuoi':1, 'organization': 1, 
                                                       'dateUpdate':1, 'date':1,
                                                       'otherInfo': {'$slice': 1}}).sort([( '$natural', 1 )]):
    if(currentUser['role'] == 'superadmin'):
      status = (datetime.datetime.now() - doc['otherInfo'][0]['time']).total_seconds()
...
def get():
  ...
  for doc in db.RectifierTransformersDetails.find({}, {'_id':1,'devSerial':1,'maChuoi':1, 
                                                       'dateUpdate':1, 'date':1,
                                                       'otherInfo': {'$slice': '[1:4]'}}).sort([( '$natural', 1 )]):
    print(doc)
    if(currentUser['role'] == 'superadmin'):
      status = (datetime.datetime.now() - doc['otherInfo'][0]['time']).total_seconds()
      if (status >= 300):
